#!/usr/bin/env python3
"""
Wrapper para evitar completamente las ventanas CMD
"""
...
def silent_subprocess_run(*args, **kwargs):
    """
    Ejecutar subprocess de forma completamente silenciosa
    """
    try:
        # Permitir ciertas llamadas específicas del ZKTeco
        if args and len(args) > 0:
            cmd = args[0]
            if isinstance(cmd, list) and len(cmd) > 0:
                cmd_str = str(cmd[0]).lower()
                # Permitir ping y comandos de red necesarios para ZKTeco
                if 'ping' in cmd_str or 'ipconfig' in cmd_str or 'netstat' in cmd_str:
                    # Para comandos de red, usar configuración silenciosa pero permitir salida
        ...
        # Para otros comandos, usar configuración completamente silenciosa
        if 'creationflags' not in kwargs:
            kwargs['creationflags'] = subprocess.CREATE_NO_WINDOW
...
def silent_subprocess_popen(*args, **kwargs):
    """
    Ejecutar subprocess.Popen de forma completamente silenciosa
    """
    try:
...
def silent_subprocess_call(*args, **kwargs):
    """
    Ejecutar subprocess.call de forma completamente silenciosa
    """
    try:
        # Configurar para evitar ventanas CMD
        if 'creationflags' not in kwargs:
